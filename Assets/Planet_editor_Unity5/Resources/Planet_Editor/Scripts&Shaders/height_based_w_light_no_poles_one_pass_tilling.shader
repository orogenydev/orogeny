...
Shader "Custom/height_based_w_light_no_poles_one_pass_tilling"
{
    Properties {
...
//#pragma exclude_renderers d3d11 xbox360
       ...
       //uniform float4 _LightColor0;
                ...
                v2f vert (appdata_base v) {
                 ...
                 //   OUT.uv = TRANSFORM_TEX(v.texcoord, _Tex0);
                    ...
                    OUT.uv = TRANSFORM_TEX(v.texcoord, _Tex0);
                    OUT.uv1 = TRANSFORM_TEX(v.texcoord, _Tex1);
                    OUT.uv2 = TRANSFORM_TEX(v.texcoord, _Tex2);
                    OUT.uv3 = TRANSFORM_TEX(v.texcoord, _Tex3);
                    OUT.uv4 = TRANSFORM_TEX(v.texcoord, _Tex4);
                    OUT.uv5 = TRANSFORM_TEX(v.texcoord, _Tex5);
                    OUT.uv6 = TRANSFORM_TEX(v.texcoord, _Tex6);
               ...
               {
                  lightDirection = normalize(mul(_WorldSpaceLightPos0,unity_ObjectToWorld).xyz - v.vertex.xyz);
                  attenuation = 1.0/(length(mul(_WorldSpaceLightPos0,unity_ObjectToWorld).xyz - v.vertex.xyz)) * 0.5;
  ...
  //c3 represents an average height so seems to be the main texture on the planet. Doing this for all the textures will affect performance
                    ...
                    half4 c1 = tex2D (_Tex1, fInput.uv1);
                    half4 c2 = tex2D (_Tex2, fInput.uv2);
                    half4 c3_ = tex2D (_Tex3, fInput.uv3*-.25);
                    half4 c3 = tex2D (_Tex3, fInput.uv3);
                    half4 c4 = tex2D (_Tex4, fInput.uv4);
                    half4 c5 = tex2D (_Tex5, fInput.uv5);
                    half4 c6 = tex2D (_Tex6, fInput.uv6);
